...
- ChronoCounter Lab K7P4 turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a compact browser timekeeping app called ChronoCounter Lab K7P4. It should manage stopwatch sessions, countdown timers, lap history, quick counters, focus intervals, settings, empty and error states, and every visible action should update real app state.
...
- Stopwatch Operations - ChronoCounter Lab K7P4
- Stopwatch Editor - ChronoCounter Lab K7P4
- Settings and Preferences - ChronoCounter Lab K7P4
- Empty and Error Recovery - ChronoCounter Lab K7P4

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Stopwatch Operations - ChronoCounter Lab K7P4
- surface_id: SURF_STOPWATCH_OPERATIONS
- unique_canvas_caption: Stopwatch Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Stopwatch data.
...
- data_entities: Stopwatch, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Stopwatch Editor - ChronoCounter Lab K7P4
- surface_id: SURF_STOPWATCH_EDITOR
- unique_canvas_caption: Stopwatch Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Stopwatch changes.
...
- data_entities: Stopwatch, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_STOPWATCH_OPERATIONS -> Save returns to SURF_STOPWATCH_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Settings and Preferences - ChronoCounter Lab K7P4
...
- purpose: Let users adjust Stopwatch workflow preferences, saved filters, defaults, or product settings requested by the task.
...
- entry_exit_rules: SURF_STOPWATCH_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
- design_guidance: Settings must support the requested workflow only; do not invent unrelated profile or billing areas.

SCREEN_SPEC_4:
- exact_screen_title: Empty and Error Recovery - ChronoCounter Lab K7P4
...
- purpose: Keep the Stopwatch workflow usable when data is missing, filtered away, loading, failed, or corrupt.
...
- entry_exit_rules: SURF_STOPWATCH_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Stopwatch workflow and preserves unrelated state.
...
## PRODUCT_SURFACES
1. SURF_STOPWATCH_OPERATIONS - Stopwatch Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Stopwatch data.
   Data: Stopwatch, ActivityEvent, Preference
...
2. SURF_STOPWATCH_EDITOR - Stopwatch Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Stopwatch changes.
   Data: Stopwatch, ValidationError
   ...
   Entry/exit: SURF_STOPWATCH_OPERATIONS -> Save returns to SURF_STOPWATCH_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_SETTINGS_AND_PREFERENCES - Settings and Preferences
   ...
   Entry/exit: SURF_STOPWATCH_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
...
4. SURF_EMPTY_AND_ERROR_RECOVERY - Empty and Error Recovery
   ...
   Entry/exit: SURF_STOPWATCH_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Stopwatch workflow and preserves unrelated state.
